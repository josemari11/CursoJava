/*4) Realizar un programa de ordenador que pida el número de horas trabajadas durante la
semana, y el precio al que se le paga la hora. Entonces, el programa debe calcular el sueldo
bruto, y a partir de éste los impuestos, para finalmente obtener el sueldo neto.
Condiciones:
...
*/
package MetodosDiciembre.Ejer4;
...
public class CalculoHoras {
	...
	//Extras
	private final int MAX_HORAS_CONVENIO=35;//Máximo de horas normales
	private double horasExtra;//total dinero
	private double numeroExtras;//numeros horas extras
	private final double PAGO_HORA_EXTRA=0.5;//50% cobra de más
	//Impuestos
	...
	public void LeerTotalHoras(){
		System.out.println("El número total de horas esta semana es de "+ totalHoras );
	}
	//Horas>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
